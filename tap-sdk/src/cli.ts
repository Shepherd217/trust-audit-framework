...
// ── competitive gap commands ──────────────────────────────────────────────────

// template browse + use + publish
const templateCmd = program
  .command('template')
  .description('Browse and publish job templates');

templateCmd
  .command('list')
  .description('Browse available job templates')
  .option('-c, --category <cat>', 'Filter by category')
  .option('--community', 'Show community templates only')
  .option('--json', 'Output as JSON')
  .action(async (options) => {
    const isJson = options.json || program.opts().json;
    const spinner = isJson ? null : ora({ text: chalk.cyan('Loading templates...'), spinner: 'dots' }).start();
    try {
      const params = new URLSearchParams();
      if (options.category) params.set('category', options.category);
      if (options.community) params.set('community', 'true');
      const res = await fetch(`${MOLTOS_API}/agent/templates?${params.toString()}`);
      const data = await res.json() as any;
      spinner?.stop();
      if (isJson) { console.log(JSON.stringify(data, null, 2)); return; }
      if (!data.templates?.length) { console.log(chalk.gray('No templates found.')); return; }
      console.log(moltosGradient('📋 Job Templates'));
      console.log();
      data.templates.forEach((t: any) => {
        console.log(`  ${t.icon || '🤖'} ${chalk.bold(t.name)} ${chalk.dim(`[${t.category}]`)} ${t.sample_budget ? chalk.green(`~${(t.sample_budget/100).toFixed(0)}`) : ''}`);
        console.log(`    ${chalk.dim(t.short_description || t.description?.slice(0, 80))}`);
        console.log(`    ${chalk.dim('moltos template use ' + t.slug)}`);
        console.log();
      });
    } catch (err: any) { spinner?.stop(); errorBox(err.message); process.exit(1); }
  });
...
templateCmd
  .command('use')
  .description('Apply a template — opens pre-filled job post')
  .argument('<slug>', 'Template slug')
  .option('--post', 'Immediately post the job using template defaults')
  .option('--json', 'Output as JSON')
  .action(async (slug, options) => {
    const isJson = options.json || program.opts().json;
    const spinner = isJson ? null : ora({ text: chalk.cyan(`Loading ${slug}...`), spinner: 'dots' }).start();
    try {
      const res = await fetch(`${MOLTOS_API}/agent/templates?slug=${slug}`);
      const t = await res.json() as any;
      if (!res.ok) throw new Error(t.error);
      spinner?.stop();
      if (isJson) { console.log(JSON.stringify(t, null, 2)); return; }
      infoBox(
        `${chalk.bold(t.name)} ${t.icon || ''}\n\n` +
        `${chalk.dim(t.description)}\n\n` +
        `${chalk.gray('Category:')}    ${chalk.white(t.category)}\n` +
        `${chalk.gray('Budget:')}      ${t.sample_budget ? chalk.green(`~${(t.sample_budget/100).toFixed(0)}`) : chalk.dim('flexible')}\n` +
        `${chalk.gray('Min TAP:')}     ${chalk.white(t.min_reputation || 0)}\n` +
        (t.tags?.length ? `${chalk.gray('Tags:')}        ${chalk.dim(t.tags.join(', '))}\n` : '') +
        `\n${chalk.dim('Post job:')} moltos jobs post --title "${t.name}" --budget ${t.sample_budget || 1000} --category "${t.category}"`,
        '📋 Template'
      );
    } catch (err: any) { spinner?.stop(); errorBox(err.message); process.exit(1); }
  });
...
templateCmd
  .command('publish')
  .description('Publish a job template to the community')
  .requiredOption('--name <name>', 'Template name')
  .requiredOption('--description <desc>', 'What this template does')
  .requiredOption('--category <cat>', 'Category')
  .option('--budget <n>', 'Suggested budget in credits', parseInt)
  .option('--tags <tags>', 'Comma-separated tags')
  .option('--json', 'Output as JSON')
  .action(async (options) => {
    const isJson = options.json || program.opts().json;
    const spinner = isJson ? null : ora({ text: chalk.cyan('Publishing template...'), spinner: 'dots' }).start();
    try {
      const cfg = JSON.parse(readFileSync(join(process.cwd(), '.moltos', 'config.json'), 'utf-8'));
      const res = await fetch(`${MOLTOS_API}/agent/templates`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': cfg.apiKey },
        body: JSON.stringify({
          name: options.name, description: options.description, category: options.category,
          sample_budget: options.budget, tags: options.tags?.split(',').map((t: string) => t.trim()) || [],
      ...
      spinner?.stop();
      if (isJson) { console.log(JSON.stringify(data, null, 2)); return; }
      successBox(`${chalk.bold(options.name)}\nSlug: ${chalk.cyan(data.slug)}\n${chalk.dim(data.url)}`, '✅ Template Published');
    } catch (err: any) { spinner?.stop(); errorBox(err.message); process.exit(1); }
  });
...
program
  .command('activity')
  .description('View public activity history for an agent')
  .argument('[agent-id]', 'Agent ID (defaults to yours)')
  .option('--json', 'Output as JSON')
  .action(async (agentId, options) => {
    const isJson = options.json || program.opts().json;
    const spinner = isJson ? null : ora({ text: chalk.cyan('Loading activity...'), spinner: 'dots' }).start();
    try {
      const cfg = JSON.parse(readFileSync(join(process.cwd(), '.moltos', 'config.json'), 'utf-8'));
      const target = agentId || cfg.agentId;
      const res = await fetch(`${MOLTOS_API}/agent/activity?agent_id=${target}`);
      const data = await res.json() as any;
      spinner?.stop();
      if (isJson) { console.log(JSON.stringify(data, null, 2)); return; }
      const s = data.stats || {};
      console.log(moltosGradient('📊 Agent Activity'));
      console.log();
      console.log(`  ${chalk.gray('Jobs completed:')} ${chalk.green(s.jobs_completed || 0)}  ${chalk.gray('Avg rating:')} ${s.avg_rating ? chalk.yellow('★' + s.avg_rating) : chalk.dim('n/a')}`);
      console.log(`  ${chalk.gray('Total earned:')}  ${chalk.green('
...
main();
 + (s.total_earned_usd || '0.00'))}  ${chalk.gray('Completion rate:')} ${s.completion_rate ? chalk.white(s.completion_rate + '%') : chalk.dim('n/a')}`);
      console.log();
      if (data.jobs?.length) {
        data.jobs.slice(0, 5).forEach((j: any) => {
          const icon = j.status === 'completed' ? chalk.green('✓') : j.status === 'disputed' ? chalk.red('⚠') : chalk.yellow('●');
          console.log(`  ${icon} ${chalk.white(j.title?.slice(0, 40) || 'Untitled')} ${chalk.dim('
...
main();
 + j.budget_usd)} ${j.rating ? chalk.yellow('★' + j.rating) : ''}`);
        });
      }
    } catch (err: any) { spinner?.stop(); errorBox(err.message); process.exit(1); }
  });

// ── end competitive gaps ───────────────────────────────────────────────────────
