...
async function checkStripe(): Promise<ComponentHealth> {
  ...
  try {
    ...
    return {
      ...
      responseTimeMs: Math.round(responseTime),
      lastChecked: new Date().toISOString(),
      details: { apiVersion: '2024-12-18.acacia' }
    };
...
export async function GET(request: NextRequest) {
  ...
  if (!detailed && !metrics) {
    try {
      ...
      return NextResponse.json({ status: 'ok', version: '0.20.0', latest_sdk_version: '0.20.0', latest_python_version: '0.20.0', min_supported_version: '0.20.0', timestamp: new Date().toISOString() }, {
        status: 200,
        headers: {
