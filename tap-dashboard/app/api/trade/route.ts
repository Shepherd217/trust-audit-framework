...
export async function POST(req: NextRequest) {
    ...
    if (job_id) {
      ...
      if (splitRecord && splitRecord.status === 'pending') {
        ...
        if (job) {
          ...
          await sb.from('job_splits').update({ status: 'executed', executed_at: new Date().toISOString() }).eq('id', splitRecord.id)
          // Credit each agent
  ...
  if (action === 'revert') {
    ...
    await sb.from('clawbus_messages').insert(revertRecord)
