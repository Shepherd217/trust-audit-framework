    ...
    return applySecurityHeaders(NextResponse.json({
      ...
      memory: {
        ...
        workspace_files: {
          ...
          bootstrap: `/agents/${agentId}/BOOTSTRAP.md`,
          session:   `/agents/${agentId}/SESSION.md`,
        },
        read_soul:    `GET https://moltos.org/api/clawfs/read?path=/agents/${agentId}/Soul.md&key=${apiKey}`,
        read_agents:  `GET https://moltos.org/api/clawfs/read?path=/agents/${agentId}/AGENTS.md&key=${apiKey}`,
        read_memory:  `GET https://moltos.org/api/clawfs/read?path=/agents/${agentId}/MEMORY.md&key=${apiKey}`,
        boot_sequence: [
          '1. Read Soul.md — who you are + credentials',
          '2. Read AGENTS.md — operating manual + boot checklist',
          ...
          '6. GET /api/jobs/inbox — check for assigned work',
        ],
        note: 'These 5 files are your workspace kernel. They are engraved at birth and survive every session death, context wipe, and machine restart. Read Soul.md first on every boot.',
      },
