-- Migration 026: TAP Committee Intelligence Schema (FIXED)
-- Removes partial indexes with now() which causes IMMUTABLE error

-- ============================================================================
-- 1. DISPUTE COMPLEXITY CLASSIFICATION
-- ============================================================================

CREATE TYPE IF NOT EXISTS task_category AS ENUM (
  'software', 'infrastructure', 'data_analytics', 
  'creative', 'research', 'administrative'
);

CREATE TYPE IF NOT EXISTS coordination_type AS ENUM (
  'single', 'sequential', 'parallel', 'dynamic'
);

CREATE TABLE IF NOT EXISTS dispute_complexity_scores (
...
CREATE INDEX IF NOT EXISTS idx_complexity_dispute ON dispute_complexity_scores(dispute_id);
CREATE INDEX IF NOT EXISTS idx_complexity_category_difficulty ON dispute_complexity_scores(primary_category, difficulty_rating);
CREATE INDEX IF NOT EXISTS idx_complexity_evidence ON dispute_complexity_scores(evidence_objectivity);
CREATE INDEX IF NOT EXISTS idx_complexity_classified_at ON dispute_complexity_scores(classified_at DESC);

-- ============================================================================
-- 2. COMMITTEE EXPERTISE PROFILES
-- ============================================================================

CREATE TYPE IF NOT EXISTS expertise_domain AS ENUM (
...
CREATE TYPE IF NOT EXISTS committee_tier AS ENUM (
  'observer', 'probationary', 'full_member', 'senior_expert', 'committee_lead'
);

CREATE TABLE IF NOT EXISTS committee_expertise_profiles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_expertise_agent ON committee_expertise_profiles(agent_id);
CREATE INDEX IF NOT EXISTS idx_expertise_domain_score ON committee_expertise_profiles(domain, expertise_score DESC);
CREATE INDEX IF NOT EXISTS idx_expertise_tier ON committee_expertise_profiles(current_tier, domain);
-- FIXED: Removed WHERE clause with now()
CREATE INDEX IF NOT EXISTS idx_expertise_active ON committee_expertise_profiles(last_judgment_at DESC);

-- ============================================================================
-- 3. VOTE CONFIDENCE METRICS (RBTS tracking)
-- ============================================================================

CREATE TABLE IF NOT EXISTS vote_confidence_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  dispute_id UUID NOT NULL REFERENCES disputes(id) ON DELETE CASCADE,
  agent_id UUID NOT NULL REFERENCES agents(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_vote_metrics_dispute ON vote_confidence_metrics(dispute_id, round);
CREATE INDEX IF NOT EXISTS idx_vote_metrics_agent ON vote_confidence_metrics(agent_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_vote_metrics_rbts ON vote_confidence_metrics(rbts_score DESC);
CREATE INDEX IF NOT EXISTS idx_vote_metrics_correct ON vote_confidence_metrics(outcome_correct) 
...
CREATE INDEX IF NOT EXISTS idx_history_agent_domain ON expertise_history(agent_id, domain, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_history_calibration ON expertise_history(agent_id, confidence_reported, was_correct);
CREATE INDEX IF NOT EXISTS idx_history_brier ON expertise_history(agent_id, brier_score);
...
CREATE INDEX IF NOT EXISTS idx_endorsements_endorsee ON peer_endorsements(endorsee_id, domain);
CREATE INDEX IF NOT EXISTS idx_endorsements_endorser ON peer_endorsements(endorser_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_endorsements_active ON peer_endorsements(endorsee_id, domain) 
...
CREATE TYPE IF NOT EXISTS selection_method_type AS ENUM ('rbts', 'expertise_weighted', 'random', 'manual');
...
CREATE INDEX IF NOT EXISTS idx_committee_dispute ON committee_assignments(dispute_id, round);
CREATE INDEX IF NOT EXISTS idx_committee_agent ON committee_assignments(agent_id, selected_at DESC);
CREATE INDEX IF NOT EXISTS idx_committee_method ON committee_assignments(selection_method);
