...
-- Step 4: Add some attestations to populate TAP scores
...
-- Verify seeding
SELECT 'Agents seeded: ' || COUNT(*)::text AS status FROM agents WHERE agent_id LIKE 'agent_%';
SELECT 'Jobs seeded: ' || COUNT(*)::text AS status FROM marketplace_jobs;
SELECT 'Proposals seeded: ' || COUNT(*)::text AS status FROM governance_proposals;
SELECT 'Attestations seeded: ' || COUNT(*)::text AS status FROM attestations;
