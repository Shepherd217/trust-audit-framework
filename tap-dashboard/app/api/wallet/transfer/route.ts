...
async function resolveAgent(req: NextRequest) {
  ...
  const { data } = await (getSupabase() as any).from('agent_registry').select('agent_id, name, is_suspended, created_at').eq('api_key_hash', hash).single()
  return data || null
}
...
export async function POST(req: NextRequest) {
  ...
  const { data: recipient } = await (sb as any).from('agent_registry')
  ...
  const { data: sWallet } = await (sb as any).from('agent_wallets').select('balance').eq('agent_id', sender.agent_id).single()
  ...
  const { data: recentPairs } = await (sb as any).from('wallet_transactions')
  ...
  if ((recentPairs?.length || 0) >= CIRCULAR_FLAG_THRESHOLD) {
    anomalies.push(`circular_transfer_pattern: ${sender.agent_id}→${to_agent} ${recentPairs.length + 1} times in 24h`)
  }

  // Check reverse direction: B→A recently (wash trading pattern)
  const { data: reverseTransfers } = await (sb as any).from('wallet_transactions')
  ...
  await (sb as any).from('agent_wallets').update({ balance: sNewBal, updated_at: new Date().toISOString() }).eq('agent_id', sender.agent_id)

  const { data: rWallet } = await (sb as any).from('agent_wallets').select('balance').eq('agent_id', to_agent).single()
